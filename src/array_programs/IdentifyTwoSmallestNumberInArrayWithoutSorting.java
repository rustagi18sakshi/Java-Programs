	...
	public static void main(String[] args) {
		...
		if(length >= 2) 
		{
			for (int i=0; i<length; i++) 
			{
				...
				// Checking if the new element is smaller than the smallest number. If yes, then assign it as smallest.
				if (arr[i] < smallest) 
				{
					smallest = arr[i];
				}

				// Checking if the element is greater than smallest number & smaller than 2nd largest.
				// If yes, then assign it as 2nd smallest
				if (arr[i] > smallest && arr[i] < second_smallest) 
				{
					second_smallest = arr[i];
				}
				// Checking if the previous smallest element is smaller than the already
				// existing second smallest element after smallest element is replaced by new element.
