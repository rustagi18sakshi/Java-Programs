	...
	public static void main(String[] args) {
		...
		if (length >= 2) {
			for (int i = 0; i < length; i++) {
				int temp = largest;
				// Checking if the element is greater than the largest number.If yes, then
				// assign it as largest
				if (arr[i] > largest) {
					largest = arr[i];
				}
				// Checking if the element is less than largest number & greater than 2nd
				// largest. If yes, then assign it as 2nd largest
